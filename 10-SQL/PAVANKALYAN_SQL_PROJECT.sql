...
select UPPER(CUSTOMER_FNAME)as  F_NAME, UPPER(CUSTOMER_LNAME) as L_NAME, CUSTOMER_EMAIL, CUSTOMER_CREATION_DATE,
case
	when o.CUSTOMER_GENDER = 'M' then concat('Mr.',concat(UPPER(CUSTOMER_FNAME),' ',UPPER(CUSTOMER_LNAME)))
    when o.CUSTOMER_GENDER = 'F' then concat('Ms.',concat(UPPER(CUSTOMER_FNAME),' ',UPPER(CUSTOMER_LNAME)))
    else ''
	end as FULL_NAME,
case
	when o.CUSTOMER_CREATION_DATE < '2005-01-01' then 'A'
    when o.CUSTOMER_CREATION_DATE >= '2005-01-01' and o.CUSTOMER_CREATION_DATE < '2011-01-01' then 'B'
    when o.CUSTOMER_CREATION_DATE >= '2011-01-01' then 'C'
    else ''
    end as CUSTOMER_CATEGORY
from online_customer as o;
...
select PROD.PRODUCT_ID, PROD.PRODUCT_DESC, PROD.PRODUCT_QUANTITY_AVAIL, PROD.PRODUCT_PRICE, ORD.PRODUCT_QUANTITY * PROD.PRODUCT_PRICE as INVENTORY_VALUES,
case
	when PROD.PRODUCT_PRICE > 200000 then PROD.PRODUCT_PRICE*0.2
    when PROD.PRODUCT_PRICE > 100000 then PROD.PRODUCT_PRICE*0.15
    when PROD.PRODUCT_PRICE <= 100000 then PROD.PRODUCT_PRICE*0.1
    else ''
    end as DISCOUNT
FROM PRODUCT PROD
...
select PC.PRODUCT_CLASS_CODE, PC.PRODUCT_CLASS_DESC, COUNT(*) as QUANTITY,(P.PRODUCT_QUANTITY_AVAIL*P.PRODUCT_PRICE) AS INVENTORY_VALUES
FROM PRODUCT P 
...
select OC.CUSTOMER_ID, concat(UPPER(OC.CUSTOMER_FNAME),' ',UPPER(OC.CUSTOMER_LNAME)) as FULL_NAME, OC.CUSTOMER_EMAIL, OC.CUSTOMER_PHONE, A.COUNTRY
...
SELECT DISTINCT S.SHIPPER_NAME,A.CITY,COUNT(*) OVER(PARTITION BY OC.CUSTOMER_ID) NO_OF_CUST,
COUNT(*) OVER(PARTITION BY S.SHIPPER_ID,OH.ORDER_ID) NO_OF_CONS
FROM SHIPPER S INNER JOIN  ORDER_HEADER OH ON S.SHIPPER_ID=OH.SHIPPER_ID 
...
SELECT P.PRODUCT_ID, P.PRODUCT_DESC, P.PRODUCT_QUANTITY_AVAIL, OH.ORDER_STATUS, P.PRODUCT_PRICE*P.PRODUCT_QUANTITY_AVAIL AS QUANTITY_SOLD, P.PRODUCT_QUANTITY_AVAIL AS QUANTITY_AVAILABLE,
COUNT(*) AS INVENTORY_STATUS,
case 
	when OH.ORDER_DATE IS NULL then 'No Sales in past, give discount to reduce inventory'
    END
FROM PRODUCT P INNER JOIN PRODUCT_CLASS PC ON P.PRODUCT_CLASS_CODE = PC.PRODUCT_CLASS_CODE
INNER JOIN (SELECT * FROM ORDER_HEADER) AS OH;
...
SELECT A.ORDER_ID,(C.LEN*C.WIDTH*C.HEIGHT) AS VOLUME
FROM ORDER_ITEMS A  INNER JOIN PRODUCT B ON A.PRODUCT_ID=B.PRODUCT_ID
INNER JOIN CARTON C ON (B.LEN*B.WIDTH*B.HEIGHT)=(C.LEN*C.WIDTH*C.HEIGHT)
WHERE (C.LEN*C.WIDTH*C.HEIGHT) <= (SELECT D.LEN*D.WIDTH*D.HEIGHT FROM CARTON D WHERE D.CARTON_ID=10);
...
SELECT OC.CUSTOMER_ID,concat(UPPER(OC.CUSTOMER_FNAME),' ',UPPER(OC.CUSTOMER_LNAME)) as FULL_NAME,SUM(OI.PRODUCT_QUANTITY) OVER(PARTITION BY OH.ORDER_ID) AS TOTAL_QUANTITY, 
...
INNER JOIN PRODUCT P ON OI.PRODUCT_ID=OI.PRODUCT_ID
...
SELECT p.PRODUCT_ID,p.PRODUCT_DESC, SUM(OI.PRODUCT_QUANTITY) OVER(PARTITION BY P.PRODUCT_ID) AS TOTAL_QUANTITY
FROM ONLINE_CUSTOMER OC INNER JOIN ORDER_HEADER OH ON OC.CUSTOMER_ID=OH.CUSTOMER_ID
INNER JOIN ORDER_ITEMS OI ON OH.ORDER_ID=OI.ORDER_ID
INNER JOIN PRODUCT P ON OI.PRODUCT_ID=OI.PRODUCT_ID
WHERE P.PRODUCT_ID=201 AND OC.CUSTOMER_ID IN (SELECT B.CUSTOMER_ID FROM ORDER_HEADER A INNER JOIN ONLINE_CUSTOMER B 
ON A.CUSTOMER_ID=B.CUSTOMER_ID INNER JOIN ADDRESS C ON C.ADDRESS_ID=B.ADDRESS_ID WHERE A.ORDER_STATUS='Shipped' AND C.CITY NOT IN ('Bangalore','New Delhi') )
...
SELECT OH.ORDER_ID,OC.CUSTOMER_ID,concat(UPPER(OC.CUSTOMER_FNAME),' ',UPPER(OC.CUSTOMER_LNAME)) as FULL_NAME,
(SUM(OI.PRODUCT_QUANTITY) OVER(PARTITION BY OH.ORDER_ID)) AS TOTAL_QUANTITY
...
INNER JOIN ORDER_ITEMS OI ON OH.ORDER_ID=OI.ORDER_ID
WHERE A.PINCODE NOT LIKE '5%' AND (OH.ORDER_ID %2=0);
